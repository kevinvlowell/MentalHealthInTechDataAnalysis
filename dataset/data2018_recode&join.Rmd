...
suppressWarnings(data2018 <- read_csv("2018_mh_data_original.csv"))

data2018_joinrecode <- data2018 %>% rename(c(
...
data2018_joinrecode$negative_impact_of_revealing_mhd_to_clients_contacts <- recode(data2018_joinrecode$negative_impact_of_revealing_mhd_to_clients_contacts,
                                                                                   "Negatively" = "Yes", 
                                                                                   "Positively" = "No", 
                                                                                   "No change" = "No")
...
data2018_joinrecode <- data2018_joinrecode %>% transmute(
self_employed,
company_size,
tech_company,
...
## Split into demographic and survey groups

```{r ch3}

data2018_demographics <- data2018_joinrecode %>% select(1:20)

data2018_survey <- data2018_joinrecode %>% select(21:45)

```


## Add year variable to 2016 and 2018 data, and then join demographic and survey data based on shared variables

```{r ch4}

#add year variable to 2016 demographic and survey datasets
data2016_survey <- data2016_survey %>% mutate("year" = "2016") %>% select(year, everything())
data2016_demographics <- data2016_demographics %>% mutate("year" = "2016") %>% select(year, everything())

#add year variable to 2018 demographic and survery datasets
data2018_survey <- data2018_survey %>% mutate("year" = "2018") %>% select(year, everything())
data2018_demographics <- data2018_demographics %>% mutate("year" = "2018") %>% select(year, everything())
