...
### Bias in discussing mental health or physical health vs. Age, colored by Company Size
...
to_display <- dat2016 %>% 
  select(gender, age, country_of_work, self_employed, had_mhd_past, family_history_of_mhd, employer_has_discussed_mh_formally, would_discussing_mh_w_employer_have_negative_effects, would_discussing_ph_w_employer_have_negative_effects, company_size) %>%
...
bscols(
  ...
  d3scatter(shared_dat, ~jitter(mh_bias), ~jitter(age), ~ factor(company_size, levels = lels),  
          width = 300,
          x_label = "Would discussing a mental health disorder with your employer have negative consequences?", 
          y_label = "Age", x_lim = c(-0.2, 1.3),
          height = 350),
  
  d3scatter(shared_dat, ~jitter(ph_bias), ~jitter(age), ~ factor(company_size, levels = lels),  
          width = 300,
          x_label = "Would discussing a physical health issue with your employer have negative consequences?", 
          y_label = "Age", x_lim = c(-0.2, 1.3),
          height = 350)),
